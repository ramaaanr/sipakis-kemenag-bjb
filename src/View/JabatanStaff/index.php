...
$(document).ready(function() {
  ...
  function fetch(status = "DISETUJUI") {
    ...
    $('#table').DataTable({
      ajax: {
        url: `/jabatan-staff`, // URL to fetch data from
...
edit
  ...
  $('#table tbody').on('click', '.delete-btn', function() {
    var id = $(this).data('id');
    Swal.fire({
      title: 'Yakin ingin menghapus Jabatan Staff ini?',
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        $.ajax({
          url: '/jabatan-staff/' + id, // Include id in the URL
          type: 'DELETE',
          success: function(res) {
            const response = JSON.parse(res);
            if (response.status) {
              $('#table').DataTable().ajax.reload(); // Reload DataTable data
              Swal.fire('Deleted!', 'Jabatan Staff berhasil dihapus.', 'success');
            } else {
              Swal.fire('Error', 'Terjadi kesalahan saat menghapus Jabatan Staff', 'error');
            }
          },
          error: function() {
