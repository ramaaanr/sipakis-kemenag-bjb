...
<div id="addModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full hidden">
  <div class="relative top-20 mx-auto p-5 border w-1/3 shadow-lg rounded-md bg-white">
    ...
    <form id="addForm">
      ...
      <input type="hidden" id="lembaga_pendidikan_id" name="lembaga_pendidikan_id" />
      <div class="mb-4">
        <label for="lembaga_pendidikan" class="block text-sm font-medium text-gray-700">Lembaga Pendidikan</label>
        <input type="text" id="lembaga_pendidikan" name="lembaga_pendidikan" readonly
          class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring focus:ring-opacity-50">
      </div>
...
$('#btn-add').on('click', function() {
  ...
  populateSelect('/jabatan-staff', '#jabatan_staff_id');
});
...
$('#addForm').on('submit', function(e) {
  ...
  $.ajax({
    url: '/staff', // Adjust the URL to your endpoint
    method: 'POST',
    data: $(this).serialize(),
